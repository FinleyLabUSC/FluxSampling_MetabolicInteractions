function costlessGrowth(saveFileName,modelsFile,CSourceFile,minMedFile,numSpecies,numCSources,isAerobic,ensembleThreshold, simStart,simEnd)
...
for i = simStart:simEnd
    ...
    [growthRatesAlone(i,:),metListAlone,secMatAlone,absMatAlone,~,absMetsAlone,secMetsAlone,absFluxesAlone,secFluxesAlone] = getSecAbsMetsMSAV_Sampling(modelsTestAlone,zeros(1,nS),isAerobic, ensembleThreshold, 75);
    ...
    if ~isempty(find(growthRatesAlone(i,:), 1)) && ~isempty(newMetsAlone)
        ...
        while ~isempty(newMetsOld)
            ...
            [growthRatesNew,metListNew,secMatNew,absMatNew,~,absMetsNew,secMetsNew,absFluxesNew,secFluxesNew] = getSecAbsMetsMSAV_Sampling(modelsTestNew,growthRatesOld,isAerobic, ensembleThreshold, 75);
